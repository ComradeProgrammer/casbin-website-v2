...
keywords: [k8s-getekeeper, Kubernetes, k8s]
---
...
- It is fledxible. Arbitrary rules can be made on any k8s resource which can be explored with kubectl getekeeper
...
:::caution
these methods are only for user to try K8s-gatekeeper, and it is not secure. If you want to use it in productive environment, please make sure you read  [Chapter 5. Advanced setting](#5-advanced-settings) and make modifications accordingly when necessary before installation 
:::
...
:::info
if you are using minikube, please execute `eval $(minikube -p minikube docker-env)` before running docker build*
:::
